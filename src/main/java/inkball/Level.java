...
public class Level {
    ...
    // Method to handle ball-to-ball collisions
private void handleBallCollisions() {
    for (int i = 0; i < balls.size(); i++) {
        ...
        for (int j = i + 1; j < balls.size(); j++) {
            ...
            if (distance < (ballA.getRadius() + ballB.getRadius())) {
                resolveCollision(ballA, ballB);  // Handle collision resolution
            }
        }
    }
}
    ...
    private void handleHoleCollisions(Ball ball) {
        for (Hole hole : holes) {
            if (hole.captures(ball)) {
                handleBallCapture(ball, hole); // Handle capture and score update
                return; // Exit the method immediately after capturing the ball
            } else if (hole.attracts(ball)) {
                hole.applyAttraction(ball, app); // Shrink the ball as it moves toward the hole
            }
        }
    }
    ...
    private void handleBallCapture(Ball ball, Hole hole) {
        // Get the score change based on the ball and hole
        int scoreChange = hole.calculateScoreChange(ball, app);
        
        // Update the score in the App
        app.updateScore(scoreChange);  // This will increase or decrease the score
    
        if (scoreChange < 0) {
            // If the score decreases (wrong hole), maybe respawn the ball or something
            nextBalls.add(getColorName(ball.getColor())); // Example behavior
        }
    
        // Remove the ball after capture
        ball.remove();
